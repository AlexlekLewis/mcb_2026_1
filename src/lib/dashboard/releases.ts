...
export const RELEASES: Release[] = [
  {
    ...
    items: [
      ...
      "PROMO LIVE — the seasonal modal ships ENABLED with the signed-off offer (2026-09-02): pre-Christmas installation urgency, no discount. Two order-by dates because plantation shutters carry a longer production lead time — shutters 30 Sept 2026, all other products 30 Nov 2026 — each shown with a live days-remaining count. Urgency is framed on availability ('installation dates are allocated as orders are confirmed, and pre-Christmas spots are filling fast') rather than a slot count, which would be an unverifiable claim. A deadline is filtered out the moment it passes, so from 1 Oct the shutters line disappears instead of advertising a dead cut-off, and the campaign self-retires on 1 Dec with no separate end date to keep in sync. All date logic runs on the Melbourne calendar date via Intl, not the visitor's clock, which also sidesteps the AEST->AEDT switch on 2026-10-04 falling between the two deadlines. Layout is a flex column with a PINNED footer: header band, scrolling middle, CTA fixed at the bottom. Caught on review — at 1366x650 (a very common laptop viewport) the CTA previously sat 250px below the fold inside the modal, so the button was unreachable without scrolling; it now sits fully in view at every size tested (1440x900, 1366x650, 375x812). Body copy tightened at the same time so the order-by dates arrive sooner. Watch quote submissions against the 4-per-30-days baseline, and promo_modal_cta_click vs _shown.",
